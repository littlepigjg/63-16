...
export default function Dashboard() {
  ...
  const totalConfigs = projects.reduce(
    (sum, p) => sum + p.environments.reduce((s, e) => s + e.configs.length, 0),
    0
  );
  const inheritedProjects = projects.filter((p) => !!p.parentId).length;
  const rootProjects = projects.filter((p) => !p.parentId).length;
  ...
  return (
    <div className="animate-slide-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <StatCard icon={Layers} label="根项目" value={rootProjects} color="sky" />
        <StatCard
          icon={GitBranch}
          label="继承项目"
          value={inheritedProjects}
          color="blue"
        />
        <StatCard icon={Activity} label="配置项总数" value={totalConfigs} color="indigo" />
              ...
              {projects.map((project) => {
                ...
                return (
                  <div
                    key={project.id}
                    onClick={() => navigate('/configs')}
                    className="flex items-center justify-between p-3 bg-[#0F172A] rounded-lg hover:border-emerald-500/30 border border-transparent transition-colors cursor-pointer"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {project.parentId ? (
                          ...
                          <Badge variant="default">根项目</Badge>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 ml-2 flex-shrink-0 flex-wrap justify-end">
                      {project.environments.map((env) => (
                        <span
                          key={env.name}
                          className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium border ${envColor(
                            env.name
                          )}`}
                        >
                          {envLabel(env.name)} (
                          {env.configs.filter((c) => c.key !== '_init').length})
                        </span>
                      ))}
                    </div>
                  </div>
          ...
          {recentLogs.length === 0 ? (
            ...
            <div className="space-y-2">
              {recentLogs.map((log) => (
                <div
                  key={log.id}
                  className="flex items-start gap-3 p-2.5 rounded-lg hover:bg-[#0F172A] transition-colors"
                >
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span
                        className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium ${logTypeColor(
                          log.type
                        )}`}
                      >
